...
begin
  ...
  if SuccessLoad=False then begin
      MessageDlg('ОШИБКА В ЗАГРУЗКЕ КЛЮЧА!',mtError,[mbOk],0);
      Close;
  end;
end;
...
begin
  StatusBar.Panels[0].Text:='Строка '+IntToStr(Memo1.CaretPos.X)+
  ', столбец '+IntToStr(Memo1.CaretPos.Y);
end;

procedure TformMain.Memo1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
   case Key of VK_LEFT,VK_RIGHT,VK_UP,VK_DOWN: StatusBar.Panels[0].Text:='Строка '
   +IntToStr(Memo1.CaretPos.X)+', столбец '+IntToStr(Memo1.CaretPos.Y);
   end;
end;
